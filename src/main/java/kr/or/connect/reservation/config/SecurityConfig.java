...
//   그 외의 요청은 모두 인증 후 접근 가능합니다.
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.csrf().disable().authorizeRequests()
				.antMatchers("/", "/main", "/users/loginerror", "/api/categories", "/api/displayinfos/**", "/api/promotions").permitAll()
				.antMatchers("/securepage", "/users/**", "/api/reservationInfos", "/api/comments**").hasRole("USER")
				.anyRequest().authenticated()
				.and().formLogin().loginPage("/users/loginform").usernameParameter("userId")
				.passwordParameter("password").loginProcessingUrl("/authenticate")
				.failureForwardUrl("/users/loginerror?login_error=1").defaultSuccessUrl("/", true).permitAll().and()
